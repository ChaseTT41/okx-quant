    ...
    def generate_signals(self, market_data: Dict[str, pd.DataFrame]) -> List[Dict]:
        ...
        for symbol, df in market_data.items():
            ...
            if vol_confirmed:
                if buy_votes >= 1:
                    buy_votes += 1
                    reasons.append(f"量比 {vol_ratio:.1f}x ≥ {volume_ratio_threshold}x ✓")
                elif sell_votes >= 1:
                    sell_votes += 1
                    reasons.append(f"量比 {vol_ratio:.1f}x ≥ {volume_ratio_threshold}x ✓")
            else:
                reasons.append(f"量比 {vol_ratio:.1f}x < {volume_ratio_threshold}x 缩量假突破")

            # ━━━ 决策 ━━━
            if buy_votes >= vote_threshold and is_bullish:
                confidence = 0.85 if buy_votes >= 3 else 0.65
                # 盘整市降仓
                if is_ranging:
                    confidence *= 0.3
                size = max_pos_pct / 100 * confidence  # 归一化
                ...
                signals.append({
                    ...
                    "reasons": reasons,
                    "risk_level": "low" if buy_votes >= 3 else "medium",
                    "suggested_size": size,
                    "stop_loss": stop_price,
                    "take_profit": tp_price,
                })

            elif sell_votes >= vote_threshold and is_bearish:
                confidence = 0.85 if sell_votes >= 3 else 0.65
                if is_ranging:
                    confidence *= 0.3
                ...
                signals.append({
                    ...
                    "confidence": min(confidence, 0.95),
                    "reasons": reasons,
                    "risk_level": "low" if sell_votes >= 3 else "medium",
                    "suggested_size": size,
                    "stop_loss": stop_price,
                    "take_profit": tp_price,
                })
...
5. 波动率过滤: ATR(14) > 近20日均值的1.5倍 → 减仓至50%""",
    ...
    def generate_signals(self, market_data: Dict[str, pd.DataFrame]) -> List[Dict]:
        ...
        for symbol, df in market_data.items():
            ...
            # 做多信号: 价格触及下轨 + RSI超卖
            near_lower = current_price <= lower * 1.01
            is_oversold = rsi <= rsi_os
            # 下影线检测 (买方开始反击)
            has_bull_wick = (close[-1] - low[-1]) > (current_price * 0.005)
            ...
            # 做空信号: 价格触及上轨 + RSI超买
            near_upper = current_price >= upper * 0.99
            is_overbought = rsi >= rsi_ob
            has_bear_wick = (high[-1] - close[-1]) > (current_price * 0.005)
